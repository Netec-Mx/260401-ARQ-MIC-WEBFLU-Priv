...
@Configuration class SecurityConfig {
 ...
 @Bean SecurityWebFilterChain security(ServerHttpSecurity http){ return http.csrf(ServerHttpSecurity.CsrfSpec::disable).authorizeExchange(a->a.pathMatchers("/accounts/status").permitAll().pathMatchers("/accounts/admin/**").hasAuthority("SCOPE_ROLE_ADMIN").anyExchange().authenticated()).oauth2ResourceServer(o->o.jwt(j->{})).build(); }
}
